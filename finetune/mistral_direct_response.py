        ...
        for inputs in dataloader:
            if metric_key_prefix == 'test':
                ...
                for untokenized_text in untokenized_inputs:
                    ...
                    output_component = split_text[1].split('<sys>')[1].replace('</s>','').strip()
...
class MistralTrainer():
    ...
    def __init__(self, model_type):
        ...
        self.key_name = "mistral-response-pred"
    ...
    def train(self):
        train_set = CustomMwozDataset(self.tokenizer, data_filename=f'{constants.DATA_DIR}train.json', model_type='mistral', mode='train', response_pred=True).data.shuffle(seed=constants.SEED)
        validation_set = CustomMwozDataset(self.tokenizer, data_filename=f'{constants.DATA_DIR}valid.json', model_type='mistral', mode='eval', response_pred=True).data.shuffle(seed=constants.SEED)
        ...
        test_set = CustomMwozDataset(self.tokenizer, data_filename=f'{constants.DATA_DIR}test.json', model_type='mistral', mode='infer', response_pred=True).data
